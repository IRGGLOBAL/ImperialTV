  ...
  Future<dynamic> requestAPi(String endPoint,
    ...
    if (isInternet) {
      ...
      if (GetStorage().read('userToken') != null) {
        debugPrint('Token: ${GetStorage().read('userToken')}');
      } else {
      ...
      debugPrint("Data : ");
      if(isMultiPart){
        debugPrint( '${multiPartData!.fields} ${multiPartData!.files} ' );
      }else{
      ...
      var options = Options(
          headers: {
            ...
            "Authorization": GetStorage().read('userToken') == null
                ? ''
                : 'Bearer ${GetStorage().read('userToken')}',
          },
